...
@Component({
  selector: 'app-auscultacion-one',
  templateUrl: './auscultacion-one.component.html',
  styleUrls: ['./auscultacion-one.component.css'],
  providers: [  ProtocolosEndocrinosService, 
    { provide: AlertConfig }],
})
export class auscultacionOneComponent implements OnChanges {
  ...
  async llenarArrayRespuestas(){    
    ...
    if (this.arrayRespuestas.length>0){         
      ...
      for await (let eva of this.arrayEvaluaciones){
        ...
        idResp = await this.buscarIDrespuesta('Observacion', posibleResp);
        respuesta= {
